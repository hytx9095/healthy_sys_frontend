import {Avatar, Button, Empty, Input, List, message, Spin} from 'antd';
...
import {
  ...
  getDoctorDialogueDoctorUsingPost
...
const ConsultCenter: React.FC = () => {
  ...
  // 联系人数据
  const [contacts, setContacts] = useState<API.DoctorContact[]>();
  // 当前选中的联系人
  const [currentContact, setCurrentContact] = useState<API.DoctorContact | null>(null);
  // 消息数据
  ...
  const loadUserContacts =  async () => {
    const res = await getDoctorDialogueDoctorUsingPost();
    if (!res.data) {
  ...
  // 加载联系人消息
  const loadMessages = async (contact: API.DoctorContact) => {
  ...
  const userAsk = async (userInput: string) => {
    ...
    try {
      const res = await askUsingPost({
        content: userInput,
        doctorId: currentContact?.doctorId
      });
      if (res.code !== 0) {
        message.error('询问失败');
      }
      ...
      <div className="message-container">
        ...
        <div className="contact-list">
          ...
          <List
            ...
            renderItem={contact => (
              <List.Item
                className={`contact-item ${currentContact?.doctorId === contact.doctorId ? 'active' : ''}`}
                onClick={() => loadMessages(contact)}
              >
                <div className="contact-info">
                  <Avatar src={contact.doctorAvatar} size={48} />
                  <div className="contact-detail">
                    <div className="contact-name">{contact.doctorName}</div>
          ...
          {currentContact ? (
            <>
              <div className="chat-header">
                <Avatar src={currentContact.doctorAvatar} size={40} />
                <div className="chat-title">{currentContact.doctorName}</div>
              </div>
              ...
              >
                ...
                {messages.length === 0 && !loadingMore ? (
                  ...
                  <List
                    dataSource={messages}
                    renderItem={(item) => (
                      <List.Item className={`message-item ${item.isDoctor ? 'user-message' : 'doctor-message'}`}>
                        <div className="message-content">
                          <div className="message-avatar">
                            {item.isDoctor && <Avatar
                              src={currentContact?.doctorAvatar}
